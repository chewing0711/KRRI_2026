"""
live_can_inference_node.py (실차 및 CAN 시뮬레이터 연동 실시간 고장 진단 노드)

설명:
실제 차량 또는 PC의 CAN 시뮬레이터가 송신하는 RAW CAN 패킷을 SocketCAN 인터페이스를 통해 
실시간 수신하여 can_parser 및 decoding_functions로 물리 디코딩을 수행하고,
1초마다 수집된 신호를 기반으로 하이브리드 고장 진단 모델(GRU) 추론 결과를 실시간 출력합니다.

실행 방법:
python live_can_inference_node.py --interface socketcan --channel can0 --speed 20 --grade 0
"""
...
sys.path.insert(0, CURRENT_DIR)  # HW 폴더 (can_parser, decoding_functions 로드용)
sys.path.insert(0, os.path.join(CURRENT_DIR, "src"))  # 배포 패키지 내 src 폴더 참조
