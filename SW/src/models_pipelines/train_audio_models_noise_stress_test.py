"""
train_audio_models_noise_stress_test.py (SW/src/models_pipelines)

[방법론 2] 노면/풍절음 교란 및 노이즈 주입 스트레스 테스트 (Noise & Pitch Shift Stress Test)
- Test 세트에 실차 주행 중 발생하는 오디오 노이즈 (SNR 0dB, -5dB) 및 피치 변동을 주입
- 모델이 배경 소음을 외운 과대평가 모델인지, 진짜 기어 충격 펄스를 분류하는 모델인지 검증
...
def add_road_noise(audio_slice, snr_db=5.0):
...
def build_raw_dataset_with_noise(snr_db=5.0):
    ...
    for scenario_name, t_launch_can in CAN_LAUNCH_TRIMS.items():
        ...
        for start_idx in range(0, n_can - 34 + 1, 34):
            ...
            if start_idx < cut:
                ...
                # Test 세트 (실차 풍절음/노면 노이즈 SNR 5dB 강제 교란)
                noisy_slice = add_road_noise(audio_slice, snr_db=snr_db)
...
def main():
    print("\n" + "=" * 110)
    print(" [Method 2: Noise Injection Stress Test (Road/Wind Noise SNR 5dB Attack)]")
    print("=" * 110)

    X_tr_seq, X_te_seq, y_tr, y_te = build_raw_dataset_with_noise(snr_db=5.0)
    ...
    print(f"\n * Train-Test Mean Cosine Similarity Leakage Score (Perturbed): {cos_sim:.6f}")
    print("=" * 110)
    ...
    out_path = os.path.join(METRICS_DIR, "audio_noise_stress_test_experiment.json")
