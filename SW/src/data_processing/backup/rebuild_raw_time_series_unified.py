"""
rebuild_raw_time_series_unified.py (gearbox_final_suite / data_processing)

Builds a RAW TIME-SERIES UNIFIED DATASET preserving EVERY single row of official_decoded.csv
WITHOUT ANY SLIDING WINDOW AGGREGATION.

Outputs:
results/unified_raw_time_series_audio_dataset.csv

Columns:
- state: normal / abnormal
- speed_kph: target speed
...
OUTPUT_RAW_TIME_CSV = os.path.join(RESULTS_DIR, "unified_raw_time_series_audio_dataset.csv")
...
def rebuild_raw_time_series_unified():
    print("=" * 100)
    print(" 🚀 REBUILDING RAW TIME-SERIES UNIFIED DATASET (NO WINDOW AGGREGATION)")
    print("=" * 100)
    ...
    for dec_file in decoded_files:
        ...
        if target_wav and os.path.exists(target_wav):
            ...
            aud_indices = np.clip(aud_indices, 0, len(audio_trimmed) - 1)
        ...
        all_dfs.append(df_can)

    df_result = pd.concat(all_dfs, ignore_index=True)

    print("\n" + "=" * 100)
    print(f" 📊 COMPLETED REBUILDING {len(df_result)} RAW TIME-SERIES ROWS (NO WINDOWING)")
    print("=" * 100)

    df_result.to_csv(OUTPUT_RAW_TIME_CSV, index=False, encoding="utf-8-sig")
    print(f" [SUCCESS] Saved Raw Time-Series Dataset: {OUTPUT_RAW_TIME_CSV}")
    print("=" * 100)


if __name__ == "__main__":
    rebuild_raw_time_series_unified()
