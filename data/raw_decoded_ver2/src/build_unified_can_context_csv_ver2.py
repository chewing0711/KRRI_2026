...
def build_unified_dataset_for_window(window_size=34, output_csv=None, cache_csv=None):
    """지정된 window_size에 대해 3단계 최종 통합 CSV 및 2단계 캐시 CSV를 results/ 폴더에 동시 생성"""
    if output_csv is None:
        output_csv = os.path.join(DATASETS_DIR, f"unified_can_context_dataset_w{window_size}.csv")
    if cache_csv is None:
        cache_csv = os.path.join(DATASETS_DIR, f"multimodal_cache_w{window_size}.csv")
...
def run_all_requested_window_sizes():
    ...
    print("\n" + "=" * 100)
    print(" 🏆 34샘플 데이터셋 생성 최종 요약 (저장 위치: results/)")
    print("=" * 100)
    if df_34 is not None:
        print(f" 1. 34샘플 (~0.10초 윈도우): 총 {len(df_34):,} 행 x {len(df_34.columns)} 열")
        print(f"    - Unified 경로: {os.path.join(DATASETS_DIR, 'unified_can_context_dataset_w34.csv')}")
        print(f"    - Cache 경로  : {os.path.join(DATASETS_DIR, 'multimodal_cache_w34.csv')}")
    print("=" * 100)
