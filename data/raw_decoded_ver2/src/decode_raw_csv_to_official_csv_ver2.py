"""
decode_raw_csv_to_official_csv_ver2.py (gearbox_final_suite / src / data_processing)

1단계: 16개 전체 세션 폴더의 4대 원본 CAN CSV 파일들을 통합하여
누락 없는 공식 16개 디코딩 시계열 CSV (results/raw_decoded/*_official_decoded.csv)를 복원하는 스크립트.

포함 신호 (10대 CAN 핵심 채널):
1. 휠속도: WHL_SPD_FL, WHL_SPD_FR, WHL_SPD_RL, WHL_SPD_RR
2. 차체 거동: LAT_ACCEL, YAW_RATE
3. 조향 시스템: SAS_Angle, SAS_Speed
4. 운전자 조작: Accel_Pedal_Pos, Brake_Pedal_Pos
5. 샤시 제어 (초기화 및 복원): TCS_CTL, ABS_ACT, ESP_CTL, TQI_TCS
...
VER2_ROOT = os.path.dirname(DATA_PROCESSING_DIR)
...
OUTPUT_DATA_DIR = os.path.join(RESULTS_DIR, "raw_decoded")
